...
def analyze_slope(roi):
    ...
    # UPDATED: Use named argument 'sharedInputs=True'
    stats = slope.clip(roi).reduceRegion(reducer=ee.Reducer.mean().combine(ee.Reducer.minMax(), sharedInputs=True), geometry=roi, scale=30, maxPixels=1e9).getInfo()
    return {'mean': stats.get('slope_mean'), 'min': stats.get('slope_min'), 'max': stats.get('slope_max')}


def analyze_vegetation_cover(roi):
    s2 = ee.ImageCollection("COPERNICUS/S2_SR_HARMONIZED").filterBounds(roi).filterDate("2024-01-01", "2024-12-31").select(
        ['B8', 'B4']).median()
    ndvi = s2.normalizedDifference(['B8', 'B4']).rename('NDVI')
    veg_mask = ndvi.gt(0.2)
    stats = veg_mask.reduceRegion(reducer=ee.Reducer.mean(), geometry=roi, scale=30, maxPixels=1e9)
...
def analyze_distance_to_roads(roi):
    ...
    nearby_roads = ee.FeatureCollection("projects/sat-io/open-datasets/GRIP4/Africa").filterBounds(search_area)
    ...
    # UPDATED: Use named argument 'sharedInputs=True'
    stats = distance_image.clip(roi).reduceRegion(reducer=ee.Reducer.mean().combine(ee.Reducer.minMax(), sharedInputs=True),
...
def analyze_shape_metrics(roi):
    ...
    if not area or area == 0 or not perimeter or perimeter == 0: return {"area_sqm": 0, "perimeter_m": 0,
                                                                          "regularity_score": 0}
...
def calculate_suitability_score(slope_stats, road_stats, shape_metrics, veg_cover):
    ...
    final_score = ((slope_score * WEIGHTS["slope"]) + (road_score * WEIGHTS["roads"]) +
                   (shape_score * WEIGHTS["shape"]) + (veg_score * WEIGHTS["vegetation"]))
